...
import { demoUsers, type DemoUser, type LoginRole } from "./data";

type AuthState = {
  user: DemoUser | null;
  signIn: (email: string, secret: string, role: LoginRole) => { ok: true } | { ok: false; error: string };
  signInAs: (userId: string) => void;
  signOut: () => void;
};

const CARD_LABEL: Record<LoginRole, string> = { admin: "Director / Admin", staff: "Teacher / Staff", parent: "Parent / Family" };

/** Which sign-in card a given account uses. */
export function loginRoleOf(role: DemoUser["role"]): LoginRole {
  return role === "owner" || role === "director" ? "admin" : role;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn: AuthState["signIn"] = (email, secret, role) => {
